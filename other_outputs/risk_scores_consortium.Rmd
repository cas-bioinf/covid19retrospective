...
data_risk <- data$patient_data %>%
  ...
  select(-age_norm, -high_creatinin, -high_pt_inr, -low_albumin, -heart_problems, -lung_problems, -markers_out_of_range, -antibiotics, -macrolides) %>%
  #avoid including indicators that are not available at hospitalization 
  select(-discontinued_medication, -last_record, -outcome,  -best_supportive_care_from,
         -starts_with("ever_"), -starts_with("any_"), -starts_with("worst"), -had_invasive,
