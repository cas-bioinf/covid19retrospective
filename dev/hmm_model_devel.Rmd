...
xx <- rstan::extract(bfit$brmsfit$fit, "observation_probs")$observation_probs
dim(xx)
colnames(xx)
nrow(as.data.frame(t(xx)))
```
...
data_grein <- readxl::read_excel(here::here("public_data", "Grein_et_al.xlsx"), na = c("")) %>%
  ...
  transmute(
    ...
    day = as.integer(day)) %>%
  group_by(patient) %>%
  mutate(breathing = if_else(breathing == "ND", breathing[which.max(day[breathing %in% c("AA", "LoO2", "HiO2", "NIPPV", "MV", "ECMO")])], breathing)) %>%
  ungroup() %>%
  mutate(
    breathing = factor(breathing, levels = c("Discharge", "AA", "LoO2", "HiO2", "NIPPV", "MV", "ECMO", "Death"), ordered = TRUE)
  )
