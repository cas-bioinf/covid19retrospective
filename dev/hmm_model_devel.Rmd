...
```{r}
data <- hmm_simulator(10, 20, 2, use_noisy_states = TRUE)
cc <- make_stancode_hmm(data$observed_structured$formula, serie_data = data$observed_structured$serie_data, rate_data = data$observed_structured$rate_data, state_data = data$observed_structured$state_data)
...
all_data <- c(make_standata( make_brms_formula_hmm(data$observed_structured$formula), data = prep_data$brmsdata),
              prep_data$standata)

sampling(model2, data = all_data)
