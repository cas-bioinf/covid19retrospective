...
```{r upsettreatment, fig.cap = "Upset plot of treatment combinations - each vertical bar displays the number of patients that received the combination indicated by filled dots in the matrix. Horizontal bars show the total number of patients receiveing the given treatment.", fig.width=8, fig.height=6 }
...
UpSetR::upset(upset_data, sets = names(upset_data), text.scale = 2)
```
...
```{r hospitaloutcomes,fig.cap="Number of patients and outcomes at the individual sites. The numbers above bars are the exact counts. Hospitalized = still hospitalized at the end of data collection at the site or transferred to other site and lost to followup. Sites are anonymized to preserve patient privacy."}
data$patient_data %>% mutate(
