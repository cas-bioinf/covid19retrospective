...
```
Our code is setup so that the formula can combine predictors drawn from the description of rates (see Table \@ref(tab:statessimple)) and those taken from patient's data. There is a separate coefficient for each rate and then the other predictors act uniformly on each rate group. The `took_XXX` variables are boolean predictors that set to true for all days since the first dose of the treatment was given - i.e. we assume the treatment alters the disease overall disease progression irreversibly.
...
hmm_hypothesis_res_list[["trt_group_m1"]] %>% select(hypothesis, point_estimate, ci_low, ci_high) %>%
  rename(`95% CI Low` = ci_low, `95% CI High` = ci_high)
```
...
print_my_fit_summary(trt_group_m2_fit)
```
