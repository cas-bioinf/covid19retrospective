...
compare_auc_to_orig(acp_auc_orig, acp_auc) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
bc_auc <- rbind(
  ...
  my_auc_summary(alive_30 ~ bc0, bello_chavolla_data, outcome_note = "day0"),
  my_auc_summary(alive_12 ~ bc0, bello_chavolla_data, outcome_note = "day0"),
...
As a simple baseline we use age in years as the sole predictor and compute AUC for this.  
...
```
We see that the "age only" baseline actually outperforms all of the other scores - although there is some uncertainty prohibiting us to be very sure about it. And the second best performing score (Caramelo et al.) is actually heavily based on age. Table of the results below as the small differences are not well visible in the plot.

```{r}
all_auc %>% mutate(score = score_caption[score]) %>%
  group_by(score, subgroup) %>% 
