...
We see that the "age only" baseline performs similarly to the best performing score (Caramelo et al.) all of the other scores - there is some uncertainty prohibiting us to be very sure about which works better. This holds even if we only take the decade of age. Note that the score by (Caramelo et al.) is actually heavily based on the decade of age. Table of the results is shown below as the small differences are not well visible in the plot.
