...
((plot_marker_data(c("CRP" = "CRP"), log_transform = TRUE, max_day = max_day) + hide_axis_theme + title_theme) /
...
data$marker_data %>% 
  filter(marker %in% c("az", "hcq", "favipiravir", "convalescent_plasma"), !is.na(value), value > 0) %>%
...
(multiverse_hcq / multiverse_az) + plot_layout(heights = c(2.5,1.5))
...
## Prognostic models are not better using age only
...
An important lesson from our analysis is that the assesment of treatment efficacy from observational data is heavily sensitive to modelling assumptions while it is usually almost impossible to determine which of the models is more likely to reflect reality (if any). We believe that using multiverse analysis is an appropriate way to explore data in such contexts as it lets us be transparent about this sensitivity.
