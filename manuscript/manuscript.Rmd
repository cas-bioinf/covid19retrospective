...
author: 
  - name: Martin Modrák
    ...
    corresponding: martin.modrak@biomed.cas.cz
    affiliation: Institute of Microbiology of the Czech Academy of Sciences
  - name: Paul-Christian Bürkner
    affiliation: University of Stuttgart
  - name: Tomáš Sieger
    affiliation: cvutCyber
  - name: Tomáš Slisz
    affiliation: 
      - resp1st
      - thomayer
  - name: Martina Vašáková
    affiliation: 
      - resp1st
      - thomayer
  - name: Grigorij Mesežnikov
    affiliation: Motol
  - name: Luis Fernando Casas-Mendez
    affiliation: Motol
  - name: Jaromír Vajter
    affiliation: Motol
  - name: Jan Táborský
    affiliation: AGEL
  - name: Viktor Kubricht
    affiliation: FNKV
  - name: Daniel Suk
    affiliation: VFN
  - name: Jan Horejsek
  ...
  - name: Martin Jedlička
    affiliation: Olomouc
  - name: Adriana Mifková
    affiliation: Olomouc
  - name: Adam Jaroš
    affiliation: Homolka
  - name: Miroslav Kubiska
    affiliation: Pilsen
  - name: Jana Váchalová
    affiliation: Pilsen
  - name: Robin Šín
  ...
  - name: Markéta Veverková
    affiliation: Horovice
  - name: Zbyšek Pospíšil
    affiliation: Trebic
  - name: Julie Vohryzková
    affiliation: charles2nd 
  - name: Rebeka Pokrievková
    affiliation: charles3rd
  - name: Kristián Hrušák
    affiliation: charles2nd
  - name: Kristína Christozova
    affiliation: charles2nd
  ...
  - name: Tomáš Hyánek
    affiliation: Homolka
address:
  - code: Institute of Microbiology of the Czech Academy of Sciences
    address: Institute of Microbiology of the Czech Academy of Sciences, Bioinformatics Core Facility, Vídeňská 1083, Prague, Czech Republic, 142 20
  - code: University of Stuttgart
    address: University of Stuttgart, Cluster of Excellence SimTech, 	Universitätsstraße 32, Stuttgart, Deutschland, 70569
  - code: cvutCyber
    address: Dept. of Cybernetics, Faculty of Electrical Engineering, Czech Technical University in Prague, Technická 1902/2, Prague, Czech Republic, 166 27
  - code: resp1st
    address: Department of Respiratory Medicine, 1st Faculty of Medicine, Charles University in Prague, Vídeňská 800, Prague, Czech Republic, 140 59
  - code: thomayer  
    address: Thomayer University Hospital, Vídeňská 800, Prague, Czech Republic, 140 59
  - code: Motol
    address: Motol University Hospital, V Úvalu 84/1, Prague, Czech Republic, 150 06
  - code: AGEL
    address: AGEL Hospital Nový Jičín, Purkyňova 2138/16, Nový Jičín, Czech Republic, 741 01  
  - code: FNKV
    address: Kralovské Vinohrady University Hospital, Šrobárova 1150/50, Prague, Czech Republic, 100 34
  - code: VFN
    address: General University Hospital in Prague, U Nemocnice 499/2, Prague, Czech Republic, 128 08
  - code: Olomouc
    address: Military Hospital Olomouc, Sušilovo nám. 5, Olomouc, Czech Republic, 771 11
  - code: Homolka
    address: Na Homolce Hospital, Roentgenova 37, Prague, Czech Republic, 150 00
  - code: Pilsen
    address: Department of Infectious Diseases and Travel Medicine, Faculty of Medicine in Pilsen, Charles University, University Hospital in Pilsen, Edvarda Beneše 13, Pilsen, Czech Republic, 305 99
  - code: Horovice
    address: Hořovice Hospital, K Nemocnici 1106/14, Hořovice, Czech Republic, 268 01
  - code: Trebic
    address: Třebíč Hospital,  Purkyňovo nám. 133, Třebíč, Czech Republic, 674 01
  - code: charles2nd 
    address: 2nd Faculty of Medicine, Charles University in Prague, V Úvalu 84, Prague, Czech Republic, 150 06
  - code: charles3rd
    address: 3rd Faculty of Medicine, Charles University in Prague, Ruská 2411, Prague, Czech Republic, 100 00
  - code: toronto
    address: Department of Statistical Sciences, University of Toronto,  100 St George St, Toronto, Canada, ON M5S 3G3
  - code: bioinfo
    address: Department of Bioinformatics, 2nd Faculty of Medicine, Charles University in Prague, V Úvalu 84, Prague, Czech Republic, 150 06
---

```{r setup, echo=FALSE, message=FALSE, warning = FALSE, results = "hide"}
knitr::opts_chunk$set(echo = FALSE, fig.width=8,out.width = "100%", cache=TRUE)
...
epidemiology_cz <- read_csv(here::here("public_data", "nakazeni-vyleceni-umrti-testy.csv"), col_types = cols(
  datum = col_date(format = ""),
  kumulativni_pocet_nakazenych = col_double(),
  kumulativni_pocet_vylecenych = col_double(),
  kumulativni_pocet_umrti = col_double(),
  kumulativni_pocet_testu = col_double(),
  kumulativni_pocet_ag_testu = col_double()
))

epidemiology_cz_october <- epidemiology_cz %>% filter(datum == "2020-10-01")

# hospitalized_cz <- read_csv(here::here("public_data", "hospitalizace.csv"), col_types = cols(
#   datum = col_date(format = ""),
#   pacient_prvni_zaznam = col_double(),
#   kum_pacient_prvni_zaznam = col_double(),
#   pocet_hosp = col_double(),
#   stav_bez_priznaku = col_double(),
#   stav_lehky = col_double(),
#   stav_stredni = col_double(),
#   stav_tezky = col_double(),
#   jip = col_double(),
#   kyslik = col_double(),
#   hfno = col_double(),
#   upv = col_double(),
#   ecmo = col_double(),
#   tezky_upv_ecmo = col_double(),
#   umrti = col_double(),
#   kum_umrti = col_double()
# ))
# 
# hospitalized_cz_october <- hospitalized_cz %>% filter(datum == "2020-10-01")
# hospitalized_cz_october$kum_pacient_prvni_zaznam

# epidemiology_cz_october$kumulativni_pocet_nakazenych
# epidemiology_cz_october$kumulativni_pocet_umrti
```

# Introduction 

The Covid-19 pandemic caused by severe acute respiratory syndrome coronavirus (SARS-CoV-2) has, as of June 2021, led to over 172 million cases and over 3.7 million deaths. The present study was designed and conducted during March - October 2020, when Czech Republic experienced a relatively mild first wave of the pandemic due to early and strict lockdowns. Low numbers of cases continued throughout the summer but during September and October, after most of the data collection for this study concluded, the number of cases was raising again. On October 1st 2020, Czech Republic had accumulated `r as.character(epidemiology_cz_october$kumulativni_pocet_nakazenych)` total confirmed Covid-19 cases and `r epidemiology_cz_october$kumulativni_pocet_umrti` confirmed Covid-19 related deaths [@http://zotero.org/users/5567156/items/SWNTWFLU].
...
The study was approved by the Ethical committees of `r ethics_string`, all data were collected in fully anonymized form. Data was collected between June and October 2020 for patients that were treated between March and October 2020.
...
data_table_1 %>% group_by(group) %>% 
  summarise(
    ...
    `Took hydroxychloroquine` = sum_percent(ever_hcq),
    ...
    Death = sum_percent(outcome == "Death"),
...
```{r}
median_ratios <-  data$marker_data %>% 
  filter(marker %in% c("lymphocyte_count", "d_dimer", "CRP", "IL_6")) %>%
  group_by(patient_id, marker) %>%
  filter(n() > 2) %>%
  summarise(ratio = max(value) / min(value), .groups = "drop") %>%
  group_by(marker) %>%
  summarise(med_ratio = median(ratio), .groups = "drop") %>%
  column_to_rownames("marker") %>% as.matrix() %>% round(1)
```
...
```{r progression, fig.cap="Disease progression for all patients included in the study as determined by breathing support required. Each vertical strip is a single patient, the ordering on the horizontal axis is by disease severity. Ventilated = any form of ventilation (non-invasive positive-pressure ventilation, mechanical ventilation and extra-corporeal membrane oxygenation)."}
my_paste <- function(x) {
...
fig_progression <- data_for_fig_progression %>% 
  mutate(patient_id = fct_reorder(patient_id, breathing_s,  .fun = my_paste, .desc = TRUE)) %>%
  filter(!is_outcome) %>%
  ggplot(aes(x = patient_id, y = day, fill = breathing_s)) + 
  geom_tile(width = 0.7) + 
  geom_point(data = data_for_fig_progression %>% filter(is_outcome) %>% mutate(outcome = factor(breathing_s, ordered = FALSE)),
             inherit.aes = FALSE, aes(x = patient_id, y = day, shape = outcome), size = 0.9) +
  scale_fill_viridis_d("", direction = -1) +
  scale_y_continuous("Days since hospitalization") +
  ...
  theme(axis.title.x = element_blank(), axis.ticks.x = element_blank(), axis.line.x = element_blank())

fig_progression
ggsave(fig_progression, filename = "supp_figs_production/fig2.tiff", width = 7.5)
```
...
ggsave(fig_markers, filename = "supp_figs_production/fig3.tiff", width = 7.5)
...
fig_assoc <- ggplot(gtmp,aes(y=name,x=est,xmin=low,xmax=high,color=type))+
...
fig_assoc 
ggsave(fig_assoc, filename = "supp_figs_production/fig4.tiff", width = 7, height = 4.5)
```
...
first_fig_multiverse_drugs <- 6
last_fig_multiverse_drugs <- 8
...
The results of the multiverse analysis for association between hydroxychloroquine, azithromycin and favipiravir usage and death is shown in Fig&nbsp;\@ref(fig:multiverseresultsdrugs) - here, we only show models that were not found to have immediate problems representing the data well or computational issues (see S3 file for details). Results for all models we tested are reported in  S`r supplementary_figures_multiverse` Figs, with additional details in S3 file. The results do not change noticeably when only patients from the first wave are included (S`r supplementary_figures_multiverse` Figs).
...
fig_drugs <- ((multiverse_hcq1 & hide_axis_theme) / (multiverse_az1 & theme(axis.title.x = element_blank())) / multiverse_fpv1)  + plot_layout(heights = c(0.8, 0.7,0.70)) & theme(axis.title.y = element_blank())

fig_drugs

ggsave(fig_drugs, filename = "supp_figs_production/fig5.tiff", width = 7.5, height = 7 * (7.5/8))
```
...
fig_risk <- compare_auc_to_orig(all_auc_orig,
                    all_auc, show_outcome = FALSE)

fig_risk
ggsave(fig_risk, filename = "supp_figs_production/fig6.tiff", width = 7.5)
```
